...
public class SeamCarver {
	...
	public SeamCarver(final Picture picture1) {
		if (picture1 == null) {
			throw new IllegalArgumentException("picture1");
		} else {
	...
	public double energy(int x, int y) {
		if (x == 0 || y == 0 || picture.width() - 1 == x || picture.height()-1 == y) {
			...
			int rv = Math.abs(right.getRed()-left.getRed());
			int bv = Math.abs(right.getBlue()-left.getBlue());
			int gv = Math.abs(right.getGreen()-left.getGreen());
